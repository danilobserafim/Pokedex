...
async function DetailsPokemon(id) {
    await fetch(`https://pokeapi.co/api/v2/pokemon/${id}/`).then(response => response.json()).then((data) => {
        ...
        <a href="../?page=${data.id / 60 < 1 ? 1 : Math.ceil(data.id / 60)}" id="btnBack"><img src="../assets/images/Google_material_icons_arrow_back_24px.png" alt="X" ></a>        
        ...
        <a href='./details.html?id=${id > 1 ? data.id - 1:1}'class='backPokemon positionArrows' ><img src='../assets/images/left-arrow.png'/></a>
            ...
            <h1>${data.name}</h1>
            <a href='./details.html?id=${data.id < 600 ? data.id + 1: 600}' class='nextPokemon positionArrows '><img src='../assets/images/right-arrow.png'/></a>
